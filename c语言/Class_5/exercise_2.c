...
void insert()
{
    ...
    LinkList *arrayList;
    //从小到大排序
    for (list; list != head; list = list->next)
    {
        //寻找链表中比插入数小的地方
        if (node->score > list->score)
        {

            while (list != head && node->score > list->score)
            {
                arrayList = list;
                list = list->next;
            }
...
void del()
{
    ...
    LinkList *array;
    while (nextList != head && nextList->score <= t)
    {
        if (nextList->score == t)
        {
            array = list;
...
{
    printf("链表：");
    for (LinkList *list = head->next; list != head; list = list->next)
    {
        if (list->next)
            printf("%d->", list->score);
        else
